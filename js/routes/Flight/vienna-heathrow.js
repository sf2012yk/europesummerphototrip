...
const Pt_VIELHR_10 = [48.109117380417544, 16.57563019772797];//離陸・着陸
...
const Pt_VIELHR_18 = [48.12152142855521, 16.53770997432757];//離陸開始
...
const Pt_VIELHR_41 = [48.85933222570009, 14.972799819390048];//150 チェコ国境
...
const Pt_VIELHR_126 = [51.64188365545717, 3.921065050653376];//オランダ側陸地
...
const Pt_VIELHR_147 = [51.64228965447761, 0.906316056114513];//イギリス側陸地
...
const Pt_VIELHR_154 = [51.65920529509907, 0.05189283296878048];//着陸前旋回開始
...
const Pt_VIELHR_173 = [51.46494138687028, -0.4356274424993907];//着陸
...
const Pt_VIELHR_178 = [51.464925564682, -0.45689429497084];//離陸・着陸地点
...
function goToHeathrow() {
  ...
  markerViennaAirport.closePopup(); // ← 移動前にVienna（始発）のポップアップを閉じる
  ...
  const fullPath = interpolatePolyline(VIEtoLHR , 100);// ← 数字が少ないほどスピードアップ
  ...
  function animate() {
   ...
   const index = frame;
    ...
    if (index < pathToHeathrow.length) {
      ...
      frame++;
      setTimeout(animate, 15); // ← 速度調整（数字が少ないほどスピードアップ）50座標 × 30ms = 約1.5秒
    } else {
      setTimeout(() => {
...
function HeathrowToVienna() {
  ...
  markerHeathrowTerminal3.closePopup(); // ← 移動前にHeathrowのポップアップを閉じる
  ...
  const fullPath = [...interpolatePolyline(VIEtoLHR, 100)].reverse();// ← 数字が少ないほどスピードアップ
  ...
  function animate() {
   ...
   const index = frame;
    ...
    if (index < pathToHeathrowToVienna.length) {
      trainMarker.setLatLng(pathToHeathrowToVienna[index]); // ← マーカーを移動
      map.panTo(pathToHeathrowToVienna[index], { animate: true, duration: 0.25 });
      frame++;
      setTimeout(animate, 15); // ← 速度調整（数字が少ないほどスピードアップ）
    } else {
      setTimeout(() => {
