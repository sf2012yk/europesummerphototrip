...
const FR_D471_CDGJC_LY = [48.70035720295291, 2.694273280073363]; //CDG空港分岐
const FR_D96_CDGJC_LY = [48.680356488445724, 2.7259448041472787]; //CDG空港分岐
...
const StGermainJC_LY = [48.58110743777336, 2.7352929155770704]; //高速インター分岐
...
const VergignyJC_LY = [47.98101081573711, 3.6793073085085672]; //TGVジャンクション
...
const Vézannes_LY = [47.87361598211996, 3.872624649694566];
...
const DijonJC_LY = [47.68069081134381, 4.080632487722975]; //ディジョン方面へ分岐
...
const LeCreusot_LY = [46.76531181474312, 4.499867540685522]; //LeCreusot駅
...
const MaconLoche_LY = [46.28272632422479, 4.778354089842496]; //MâconLoché駅
...
const MaconTGVJC_LY = [46.25943160251653, 4.826014040479367]; //ジュネーブ行き分岐
...
const LyonCityJC_LY = [45.882760703301294, 4.880043976295247]; //リヨン市内行き分岐
...
const Sathonay = [45.82005402913132, 4.875467629611808]; //Sathonay駅
const LyonTn_1 = [45.81356298762567, 4.86957181149089];　//リヨントンネル
const LyonTn_3 = [45.808876231121246, 4.881955300435311];　//リヨントンネル
...
const PartDieu = [45.7603030076784, 4.860375801911088]; //リヨン・パールデュー駅
...
Vézannes_LY,FR_D51_LY,FR_D226_LY,FR_D35_LY,FR_D965_LY,FR_D139_LY,FR_D944_LY,FR_D117_LY,FR_D200_LY,FR_D109_LY,FR_D956_LY,
...
function goToLyon() {
  ...
  markerParisLyon.closePopup(); // ← 移動前にParisLyon駅のポップアップを閉じる
  ...
  const fullPath = interpolatePolyline(LGV_PRLY , 50);// ← 数字が少ないほどスピードアップ
...
function goToParis() {
  ...
  markerPartDieu.closePopup(); // ← 移動前にPartDieu駅のポップアップを閉じる
  ...
  const fullPath = [...interpolatePolyline(LGV_PRLY, 50)].reverse();// ← 数字が少ないほどスピードアップ
  ...
  function animate() {
   ...
   const index = frame;
    ...
    if (index < pathToParis.length) {
      trainMarker.setLatLng(pathToParis[index]); // ← マーカーを移動
