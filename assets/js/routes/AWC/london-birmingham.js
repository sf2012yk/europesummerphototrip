...
function goToBirmingham() {
  ...
  markerEuston.closePopup(); // ← 移動前にEuston（始発）のポップアップを閉じる
  ...
  const fullPath = interpolatePolyline(AWC_ESBM , 50);// ← 数字が少ないほどスピードアップ
  ...
  function animate() {
   ...
   const index = frame;
    ...
    if (index < pathToBirmingham.length) {
      trainMarker.setLatLng(pathToBirmingham[index]); // ← マーカーを移動
...
function goToEuston() {
  ...
  markerBirmingham.closePopup(); // ← 移動前にBirminghamのポップアップを閉じる
  ...
  const fullPath = [...interpolatePolyline(AWC_ESBM, 50)].reverse();// ← 数字が少ないほどスピードアップ
  ...
  function animate() {
   ...
   const index = frame;
    ...
    if (index < pathToEuston.length) {
      trainMarker.setLatLng(pathToEuston[index]); // ← マーカーを移動
