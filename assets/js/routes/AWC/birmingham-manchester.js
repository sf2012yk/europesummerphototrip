...
function goToBirminghamtoManchester() {
  ...
  markerBirmingham.closePopup(); // ← 移動前に（始発）のポップアップを閉じる
  ...
  const fullPath = interpolatePolyline(AWC_BMMC , 50);// ← 数字が少ないほどスピードアップ
  ...
  function animate() {
   ...
   const index = frame;
    ...
    if (index < pathToBirminghamtoManchester.length) {
      trainMarker.setLatLng(pathToBirminghamtoManchester[index]); // ← マーカーを移動
